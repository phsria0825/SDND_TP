...
#### install anaconda3
1. windows로 anaconda3 홈페이지에서 anaconda3 for linux  {file name.sh}다운 후, 리눅스 폴더로 옮기기

- 리눅스 폴더에 권한 부여 방법
```
cd /{리눅스 폴더}
chmod 777 {폴더이름} # 권한 부여할 폴더명
...
#### install cuda & cudnn
- 리눅스 명령창에서 설치확인
cuda 버전이 나오는지 확인(만약 저기가 n/a가 뜬다면 윈도우에서 그래픽 카드 드라이버 업데이트 해야함)
...
- 텐서플로우에 맞는 cuda toolkit 설치 명령어 복사,
- (ex cuda toolkit 11.2)

```
## Download Installer for Linux WSL-Ubuntu 2.0 x86_64 (cuda toolkit 11.2)
...
2. cudnn 받아오기 cuda 11.2에 맞춰서 받기
https://developer.nvidia.com/rdp/cudnn-archive

- 텐서플로우와 cuda toolkit 에 맞는 cudnn .tgz 파일 다운
- 리눅스 명령창에 아래명령어 입력

```
cd {cudnn 파일 위치}
tar -xvf {다운받은 cudnn 파일 이름}
sudo cp ./cuda/include/* /usr/local/cuda-11.2/include
sudo cp -P ./cuda/lib64/* /usr/local/cuda-11.2/lib64
sudo sudo chmod a+r /usr/local/cuda-11.2/lib64/libcudnn*
...
source ~/.bashrc
```
#### tensorflow object_detection 설치전에 WAYMO-dataset pip
#### tensorflow object_detection 설치
```
conda create -n {원하는 이름} pip python=원하는 버전
...
* **Note** `pipeline_new.config`파일을 vscode로 열어서 optimizer의 종류와 learning_rate, step 등을 수정 가능 (parameter tuning) 
...
```
**Tensor bord eval-graph 생성방법**
**리눅스 터미널에서 "Waiting for new checkpoint" 의 문구가 나오면  윈도우에서 /experiments/reference/checkpoint 파일을 vscode로 들어가서 1번째 줄의 `model_checkpoint_path: "ckpt-{x}"` x를 증가시킨뒤 저장**
